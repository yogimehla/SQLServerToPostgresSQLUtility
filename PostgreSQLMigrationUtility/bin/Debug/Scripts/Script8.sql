...
where
( schema_name(schema_id)+'.'+ object_name( parent_object_id) in(select tablename from include_table_list)
...
begin
 ...
 set @TSQLCreationFK='ALTER TABLE '+ lower (case @ParentTableSchema when 'dbo' then 'public' else @ParentTableSchema end) +'.'+ lower (@ParentTableName) +'  ADD CONSTRAINT '+  replace(replace(replace(replace(replace(replace(lower (@ForeignKeyName),'Registration','reg'),'Patient','pat'),'Additional','add'),'Organisation','org') ,'f_k__','fk_'),'__','_') 
 + ' FOREIGN KEY('+ lower(lower(rtrim(ltrim(@StrParentColumn))))+') '+ char(13) +'REFERENCES '+ lower (case @ReferencedTableSchema when 'dbo' then 'public' else @ReferencedTableSchema end) +'.'+ lower ( @ReferencedTable)+' ('+ lower(ltrim(lower(rtrim(ltrim(@StrReferencedColumn)))))+');' + char(13)
 --dmd.route
 --if not exists
