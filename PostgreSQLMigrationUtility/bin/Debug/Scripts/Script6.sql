...
begin
 ...
 begin
  if @IsIncludedColumn=0 
    set @IndexColumns=@IndexColumns + 	lower ((case @ColumnName when 'order' then '"order"' when 'Default' then '"Default"'
				when 'offset' then '"offset"'
				 else @ColumnName  end) )  +','  --+ case when @IsDescendingKey=1  then ' DESC, ' else  ' ASC, ' end
  else 
   set @IncludedColumns=@IncludedColumns  + lower((case @ColumnName when 'order' then '"order"' when 'Default' then '"Default"'
				when 'offset' then '"offset"'
				 else @ColumnName  end))    +', ' 
...
set  @TSQLScripCreationIndex='ALTER TABLE '+  lower (@SchemaName) +'.'+ lower(@TableName) + ' ADD CONSTRAINT ' + replace( replace( lower (replace(replace(replace(replace(@IndexName,'Registration','reg'),'Patient','pat'),'Additional','add'),'Organisation','')),'P_K__','pk_'),'U_Q__','uq_') + @is_unique_constraint + @is_primary_key +  +  '('+@IndexColumns+') '+ 
...
select replace(command,'dbo','public') as command from @constraints