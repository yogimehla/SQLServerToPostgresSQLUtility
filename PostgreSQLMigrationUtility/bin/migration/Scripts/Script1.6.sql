Create function getuqpkData (@tableNamepar nvarchar(MAX))
	...
	begin
		...
		begin
			if @IsIncludedColumn = 0
				set @IndexColumns = @IndexColumns + dbo.udf_SpacesForCases((
							case @ColumnName
								when 'order'
									then '"order"'
								when 'Default'
				...
				set @IncludedColumns = @IncludedColumns + dbo.udf_SpacesForCases((
							case @ColumnName
								when 'order'
									then '"order"'
								when 'Default'
		...
		set @TSQLScripCreationIndex = ',' + char(13) + char(9) + 'CONSTRAINT ' + replace(replace(dbo.udf_SpacesForCases(replace(replace(replace(replace(@IndexName, 'Registration', 'reg'), 'Patient', 'pat'), 'Additional', 'add'), 'Organisation', '')), 'P_K__', 'pk_'), 'U_Q__', 'uq_') + @is_unique_constraint + @is_primary_key + + '(' + @IndexColumns + ')' + case 
				when len(@IncludedColumns) > 0
