...
begin
 ...
 begin
  if @IsIncludedColumn=0 
   set @IndexColumns=@IndexColumns + dbo.udf_SpacesForCases ( @ColumnName)  + case when @IsDescendingKey=1  then ' DESC, ' else  ' ASC, ' end
  else 
   set @IncludedColumns=@IncludedColumns  + dbo.udf_SpacesForCases (@ColumnName ) +', ' 
 ...
 set @TSQLScripCreationIndex='CREATE '+ @is_unique  + ' INDEX '  +  replace( replace(  dbo.udf_SpacesForCases (replace(replace(replace(replace(@IndexName,'Registration','reg'),'Patient','pat'),'Additional','add'),'Organisation','org')),'i_x__','ix_'),'__','_') +' ON ' + dbo.udf_SpacesForCases (@SchemaName) +'.'+ dbo.udf_SpacesForCases (@TableName) + '('+@IndexColumns+') '+ 
